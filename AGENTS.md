...
- A handler documented as `@param array{...} $params` describes the
  fields callers pass directly to `sb.api.call`. The generator unwraps
  that single array shape into `Api<Action>Request`; it must not emit a
  synthetic `{params: ...}` wrapper that does not exist on the client API.
...
- Demo payloads are opaque ~1 KiB text blobs carrying the synth
  ...
  (the banlist's desktop/mobile row actions and the player drawer all
  link to `getdemo.php?type=B&id=<bid>` when `demo_count > 0`; link →
  200 → file download with `Content-Disposition`). On-disk
  ...
  uploads share one storage shape. `api_bans_add` accepts a demo only as
  the uploader's complete `(32-character lowercase hex filename,
  original filename)` pair, verifies the regular file is still contained
  under `SB_DEMOS`, and rejects it before writing the ban when validation
  fails. The add form's historical `dfile: 0` plus empty `dname`
  payload remains the no-attachment sentinel. The download endpoint
  basenames the stored filename, rejects
  symlinks, and requires the canonical file path to remain under the
  canonical `SB_DEMOS` root before reading bytes.
...
- `htmlspecialchars_decode` / `html_entity_decode` on JSON-API params
  (nickname, reason, comment text, chat message, …) → the JSON body is raw UTF-8. The
...
- Surfacing per-row admin-authored comments behind a silent count
  ...
  (`page_bans.tpl` / `page_comms.tpl` reference). Admins also need
  the `row-action-comment-add` CTA on every punishment; each rendered
  comment carries Edit for its author or an owner and Delete for an
  owner (#1544). The shared editor is
  `partials/punishment-comment-editor.tpl`; the shared destructive
  dispatcher is `web/scripts/comment-actions.js`. The drawer's
  `[data-testid="drawer-comments"]` mirror carries the same Add /
  Edit / Delete action set so mobile users do not lose the controls.
  Don't reach for a comment count that only opens the drawer: every
  desktop row already carries the data payload the disclosure renders,
  while the drawer remains the canonical mobile expansion path.
- Bare HTML-entity glyphs (`&#9998;` ✎ / `&#10003;` ✓ / `&#8634;` ↺
  ...
  with `data-tooltip` + `aria-label` on icon-only buttons
  (`class="btn btn--ghost|secondary btn--icon btn--sm"` inside
  `.row-actions.row-actions--icons`). The bare-entity shape was what
  the v2.0 banlist row-actions cell shipped while the sibling
  commslist row used Lucide icons + text labels — the inconsistency
  read as broken icons / a different app, and screen readers
  announced the glyph code rather than the action. Density chrome
  for banlist / commslist / admin-admins is icon-only + the themed
  `data-tooltip` contract from `sb.js` (admins list is the
  reference). Icons are 14×14 inline-sized
  (`style="width:14px;height:14px"`); Remove puts
  `color:var(--danger)` on the icon, not the button. Pinned by
...
| Wire a `window.opener.<callback>(...)` slot on the parent surface of a popup file-upload page (e.g. `window.icon`, `window.demo`, `window.mapimg`) | Define `window.<callback> = function (filename) { … }` inside the parent template's inline `<script>` block (use the `{literal}…{/literal}` Smarty guard so `{` / `}` inside the JS body don't trigger Smarty parsing). The function patches the parent form's hidden input (e.g. `document.getElementById('icon_hid').value = filename;`) and any visible affordances (toggle a thumbnail preview, swap a "Choose file" label for the filename). Reference: `web/themes/default/page_admin_mods_add.tpl` + `page_admin_edit_mod.tpl` for the `window.icon` wiring (#1402), and `page_admin_edit_ban.tpl` for edit-ban's `window.demo` wiring. Add-ban is the compatibility exception: `web/pages/admin.bans.php`'s tail script defines `demo(id, name)` plus the `window.did` / `window.dname` globals consumed by `page_admin_bans_add.tpl`, so third-party add-ban templates retain the callback too. It must update `#demo.msg` with `textContent`; the original upload filename is untrusted and must never reach `innerHTML` / the removed MooTools `setHTML()` path. Without a callback slot the popup emits `<script>window.opener.icon('foo.jpg')</script>` and throws `TypeError: window.opener.icon is not a function`; the popup never closes, the parent state never updates, and the uploaded asset is orphaned on disk. The `UploadHandler` emits the call unconditionally; the parent surface must be ready for it. |
...
| Add visible row actions to a table-rendered admin list (Edit / Unmute / Remove buttons + responsive mobile-card mirror) | `web/themes/default/page_admin_admins_list.tpl` is the density reference: icon-only `<a class="btn btn--ghost btn--icon btn--sm">` / `<button class="btn btn--secondary btn--icon btn--sm">` inside `.row-actions.row-actions--icons`, with `data-tooltip` + `aria-label` (themed tooltip via `sb.js`). Public banlist / commslist (`page_bans.tpl` / `page_comms.tpl`) follow the same chrome on `table table--compact` desktop tables (tighter cell padding + 22px avatars + 1.75rem icon buttons via `.table.table--compact .row-actions--icons .btn--icon.btn--sm`), plus `.ban-card__actions` mobile rows. Wire destructive / state-changing buttons via `data-action="…"` + `data-bid` + `data-fallback-href`; the inline page-tail JS calls `sb.api.call(Actions.PascalName)` and falls back to the GET URL if the JSON dispatcher is absent. Banlist Remove points at the legacy GET handler (`?p=banlist&a=delete&id=…&key=…`) because no JSON `bans.delete` action exists yet. |
| Wire a comment-delete trash icon on any of the four comment-rendering surfaces (banlist, commslist, admin.bans protests, admin.bans submissions) | `web/scripts/comment-actions.js` (#1402). Single document-level dispatcher loaded from `core/footer.tpl` (`<script src="./scripts/comment-actions.js" defer>`) that picks up every `[data-action="comment-delete"]` click, opens a shared `#comment-delete-dialog` `<dialog>` (injected once by the script), then `sb.api.call(Actions.BansRemoveComment, { cid, ctype, page })`. The four call sites emit the trigger with `data-cid="<int>"` + `data-ctype="<B|C|S|P>"` + `data-page="<int>"` (the page number for client-side row-hide / paginator-aware redirect; `data-page="-1"` is the sentinel for unpaginated moderation queues). The `ctype` letter matches `:prefix_comments.type` (B=ban, C=comm-block, S=submission, P=protest); `api_bans_remove_comment`'s `ctype` arm consumes all four. Don't duplicate the dispatcher inline per page — the single mount point is the contract, otherwise a future bug-fix has to land in four places. Do NOT reintroduce `window.confirm()` for this surface. |
| Add or edit a punishment / moderation-queue comment from a list row or the player drawer | `web/themes/default/partials/punishment-comment-editor.tpl` is the shared B/C/S/P editor, submitted by `web/scripts/banlist.js` to `Actions.BansAddComment` / `Actions.BansEditComment`. `page.banlist.php` / `page.commslist.php` expose `comment_url` plus per-comment `cid`, `edit_url`, `can_edit`, and `can_delete`; their templates render `row-action-comment-add` on desktop + mobile and Edit/Delete inside each inline disclosure. `api_bans_detail` / `api_comms_detail` expose `comments_can_add` plus the per-comment permission booleans so `theme.js::renderOverviewPane()` mirrors the same controls in the drawer. Any web admin may add a ban or comm-block comment. Submission and protest comments require `ADMIN_BAN_SUBMISSIONS` and `ADMIN_BAN_PROTESTS`, respectively, at both page and API boundaries. Only the author or an owner may edit; only an owner may delete. The edit action binds all three identifiers (`cid`, parent `bid`, and `ctype`) before updating, so a forged parent ID cannot retarget a comment; deletion rebinds `cid` plus `ctype` after validating the stored row. Comment text is raw JSON input: never entity-decode it before editing or rendering. `encodePreservingBr()` escapes each server-rendered display segment with malformed-UTF-8 substitution; `theme.js::renderCommentText()` mirrors the escaping, legacy `<br>` preservation, and HTTP(S) linkification in the drawer; `substituteInvalidUtf8()` normalises the raw editable value before Smarty escapes the textarea. Gate "last edit" metadata on `edittime`, not the joined editor name: deleting the editor's admin row makes the name null, but the timestamp and "deleted admin" attribution must survive. |
| Add a confirm + reason modal for an irreversible row-level action (unban, lift comm block, delete admin, delete mod, …) | `web/themes/default/page_bans.tpl` (`#bans-unban-dialog`, `Actions.BansUnban`) and `web/themes/default/page_comms.tpl` (`#comms-unblock-dialog`, `Actions.CommsUnblock`) are the canonical reference (#1301), with `web/themes/default/page_admin_admins_list.tpl` (`#admins-delete-dialog`, `Actions.AdminsRemove`, #1352) and `web/themes/default/page_admin_mods_list.tpl` (`#mod-delete-dialog`, `Actions.ModsRemove`, #1397) as the third and fourth references for the optional-reason variant. Confirm-only delete (no reason field): `#bans-delete-dialog` (legacy GET navigate), `#comms-delete-dialog` (`Actions.CommsDelete`), and `#comment-delete-dialog` (injected by `comment-actions.js`). Never use `window.confirm()` / `alert()` for these row actions. Shape: a `<dialog hidden>` with a `<form method="dialog">` carrying a `<textarea aria-required="true">` (or `aria-required="false"` for the optional-reason variant — see admins-delete / mod-delete; omit the textarea for confirm-only deletes) (NOT the native `required` — that lets the browser block the form submit before our handler runs, swallowing the inline-error UX), a Cancel button, and a Confirm submit button. The page-tail JS opens the dialog via `showModal()` on `[data-action]` clicks, validates the trimmed reason on submit (load-bearing gate is server-side), forwards `ureason` to the JSON action, and on success flips the row in place via the same `flipRowToUnbanned`/`flipRowToUnmuted` helper the legacy single-click flow used (or removes the row outright + decrements the count badge for the admins-delete / mod-delete / comms-delete variants where there's no "now-unbanned" state to render). The legacy GET fallback (`?p=banlist&a=unban&id=…&key=…&ureason=…` / `?p=commslist&a=ungag…&ureason=…`) is the no-JS / hand-edited-URL path; both halves now reject empty `ureason` server-side so the audit log carries the *why*. The admins-delete and mod-delete variants have no legacy GET handler — `RemoveAdmin()` / `RemoveMod()` always went through the JSON dispatcher pre-#1123 D1 — so their `data-fallback-href` lands the operator back at the list page as a graceful no-op when the JSON dispatcher is missing entirely (third-party theme stripping `api.js`); the audit-log "Reason: …" suffix is only emitted when `ureason` is non-empty (vs always-emitted on the bans / comms variants where reason is required). **Do not** put `onclick="event.stopPropagation()"` on the trigger button — `document.addEventListener('click')` is how the dialog opener picks the click up, and stopPropagation would silently swallow it (the action button isn't inside any `[data-drawer-href]` ancestor anyway, so the defensiveness was a copy-paste from the row-name anchor that doesn't apply here). The submit button MUST flip through `setBusy(submitBtn, true)` BEFORE `sb.api.call(...)` leaves the page and clear via `setBusy(submitBtn, false)` on every non-navigating response branch — see "Loading state on action buttons" in Conventions for the contract, the inline-script local wrapper shape, and the regression guard. |
...
| Surface admin-authored per-ban / per-comm comments inline on the public banlist + commslist (`<details data-testid="ban-comments-inline">` / `comm-comments-inline`) | `web/themes/default/page_bans.tpl` + `web/themes/default/page_comms.tpl` (#BANLIST-COMMENTS). Both desktop tables wrap the player-name anchor + comment chip in `.ban-player-cell__head` so the collapsed chip sits on the SAME line as the name (icon + tabular-nums count only; `title` / `aria-label` carry "N comment(s)"). The name uses `.ban-player-cell__name` (ellipsis truncate) so the chip never wraps alone; opened details take `flex-basis: 100%` and the body lists each comment below. CSS lives under `.ban-comments-inline*` in `theme.css`. The `nofilter` on comment text is load-bearing (`encodePreservingBr` + URL-wrap). Mobile cards keep a non-interactive count indicator (`ban-comments-count-mobile`). Gate: `$view_comments`. **#1500** also hides comment author/editor when `banlist.hideadminname` is on. Regression: `BanlistCommentsVisibilityTest` + `banlist-comments-visibility.spec.ts`. |
